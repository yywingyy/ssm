...
<head>
    ...
    $(function () {
        ...
        $("#btn").click(function(){
            if($("#userName").val()==""){
                $.messager.show({title:'提醒',msg:'用户名不能为空',timeout:1000,	showType:'slide',style:{top:document.body.scrollTop+document.documentElement.scrollTop} });
                return false;
            }else if($("#password").val()==""){
                $.messager.show({title:'提醒',msg:'密码不能为空',timeout:1000,	showType:'slide',style:{top:document.body.scrollTop+document.documentElement.scrollTop} });
                return false;
            }
            ...
            $.ajax({
                ...
                success:function(result){
                    if(result.status==0){
                        ...
                        $.messager.show({title:'提醒',msg:result.message,timeout:1000,	showType:'slide',style:{top:document.body.scrollTop+document.documentElement.scrollTop}});
                    }
                },
                error:function(){
                    alert("登录异常");
...
<div class="login">
    ...
    <form method="post" id="loginForm">
        <input type="text" name="uUsername" placeholder="Username" required="required" />
        <input type="password" name="uPassword" placeholder="Password" required="required" />
        <button type="button" id="btn" class="btn btn-primary btn-block btn-large">Let me in.</button>
    </form>
</div>
